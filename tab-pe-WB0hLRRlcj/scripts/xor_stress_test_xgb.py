...
if __name__ == "__main__":
    ...
    exp_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "results", f"xor_stress_test_{num_features}_features")
    setup_logging(log_file=os.path.join(exp_folder, "log.txt"))

    priv_data = TabularCSV(
        csv_path="https://raw.githubusercontent.com/toan-vt/cloud-data-store/refs/"
        f"heads/main/tabular/sim/xor-stress-test/{num_features}_feature_xor/data_train.csv",
    ...
    tab_classifier = TabClassifier(
        test_data=test_data, model_name="xgboost", filter_criterion={VARIATION_API_FOLD_ID_COLUMN_NAME: -1}
        # NOTE: paper's official script uses model_name="tabpfn" here, but the modern `tabpfn`
        # package gates pretrained-weight downloads behind an interactive one-time license
        # acceptance (TabPFNLicenseError) with no non-interactive path available in this headless
        # environment. Substituted XGBoost (default max_depth=6, sufficient for up to 5-way XOR
        # per the paper's own App. A.1 depth-vs-correlation-order argument) since the paper itself
        # uses XGBoost-max-depth sweeps for structurally the same diagnostic elsewhere (Fig. 12).
        # Disclosed in BUGFIX_LOG.md / VERDICTS.md as a substitution, not silently swapped.
    )
